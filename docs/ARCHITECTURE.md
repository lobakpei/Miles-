...
最後更新：2026-07-23
基準版本：正式 v6.79.0；本 branch 加入 Phase 1A 純資料搬遷
...
`data/card-channels.js` 嘅 `CHANNEL_OFFERS` 以卡 ID 關聯平台加碼。`active: true` 項目必須同時有 `verified: true`、實際 `expiry` 同有效來源 URL。過期狀態由香港日期 helper 判定。第三方資料只證明渠道自己提供嘅額外獎賞／流程，唔可取代銀行規則。
