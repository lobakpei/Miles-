...
最後更新：2026-07-23（香港）
基準版本：正式 v6.79.0
本文件描述 Phase 1A 候選分支結構；正式 production `main` 喺 PR 合併前仍以 `ba8f6db0…`／v6.79.0 inline 架構運行。法律、安全及官方資料真確性規則最高，產品方向以 [`ACREMILES_20260722_DECISION_SOURCE_OF_TRUTH.md`](ACREMILES_20260722_DECISION_SOURCE_OF_TRUTH.md) 為準，目標分層見 [`ACREMILES_CURRENT_ARCHITECTURE_MAP_V1.md`](ACREMILES_CURRENT_ARCHITECTURE_MAP_V1.md)。未來方向另見 [`ROADMAP.md`](ROADMAP.md) 同 [`ARCHITECTURE-SEO.md`](ARCHITECTURE-SEO.md)。
...
- 現行 bottom nav 仍係 `計算／規劃／首頁／優惠／攻略`，一般 Planner 第一層仍有 Beginner／Advanced gateway；新決定要求 `點賺／點用／首頁／優惠／攻略`，而 Beginner／Advanced 只屬環球票內部分流。呢個差距留待後續 Phase；Phase 1 不改 UI。
- 「大額消費」同「由目的地出發」只可描述現有頁面／歷史，不再係目標產品定位。
- 目標分層係 source data → domain engine → state → UI → content／SEO；Phase 1A 只完成信用卡、渠道同來源 registry 三個資料責任，其他 engine、state、content 仍然內嵌，唔可以將整份藍圖寫成已落地。
- GitHub Pages 繼續由 `main` 部署；Draft PR 本身唔會改 production。

## 1. 系統總覽

AcreMiles 係部署喺 GitHub Pages 嘅無後端靜態 PWA。Phase 1A 候選分支已將信用卡資料抽到 `data/`；主 App、RTW 資料、文章內容、optimizer、CSS 同 UI 大部分仍然集中喺 `index.html`。
...
| 主程式 | `index.html`，Vanilla HTML／CSS／JavaScript；以 4 個 blocking data scripts 載入信用卡資料，再執行原有 inline core／UI scripts |
...
```text
/
├── index.html                 # 主 App、非卡資料、引擎、內容、CSS、UI；載入 data/
├── data/
│   ├── cards-official.js     # 銀行官方卡數值、條款分類及生成 metadata
│   ├── card-channels.js      # 渠道 records；aggregator 由 runtimeOffer records 建立舊 UI view
│   ├── source-registry.js    # 官方 audit sources、推廣核實紀錄及 unknown model assumptions
│   └── index.js              # schema 驗證、hydrate 及 browser／Node 共用 export
...
│   ├── generate-card-pages.js # 直接讀 data/ 生成 cards/
│   ├── generate-share-pages.js# 從 share-meta 生成 share/
│   ├── smoke-http.js          # 所有本地 URL／資產 HTTP 檢查
│   ├── regression-lock.js     # 卡資料／完整 index 行為／可見 surface／optimizer／產品檔／DOM／storage／generated drift lock
...
`var BM = (function(){ ... })()` 係目前最接近獨立 domain layer 嘅部分。佢由 `ACREMILES_CARD_DATA` 接收已驗證資料，並包含：

- `DEFAULT_CARDS`：由 `data/cards-official.js` 同 `data/source-registry.js` hydrate 嘅 9 張卡 runtime view。
- 每張卡嘅 `publicDetails`：App 同 card generator 直接讀取嘅長版條款分類。
- `CHANNEL_OFFERS`：由 `data/card-channels.js#records` 嘅 `runtimeOffer` records 衍生，保持 v6.79.0 UI view。
- `REDEMPTIONS`：兌換參考。
- optimizer 純函數。
- oneworld 航司、機場、距離表、分區、直航三態同 RTW evaluator。
- 單程區域／艙等表。

`scripts/verify.js` 會直接 import `data/`，再將同一份資料注入抽出嘅 core script，用 Node 執行 invariant 同快照測試；唔會用 regex 從 HTML 抽信用卡資料。
...
- 身份／生成：`id`、`slug`、`image`、`status`、`name`、`bank`、`program`。
...
- 成本：首年／續期年費、豁免、轉分費。
- 來源：`data/source-registry.js` 以同一 card ID 保存 audit 用嘅 `sourceVerifiedAt`、`sourceStatus`、`sourceDocs`；官方產品／申請 `url` 仍屬 card runtime data，保留喺 `cards-official.js`。aggregator 再 hydrate audit records 到 runtime card。
- 推薦 gate：現時主要用 `verified`；HSBC Visa Signature 為 `false`。

`applyWeeks` 係 v6.79.0 legacy timeline estimate，唔係銀行承諾。Phase 1A 候選已將佢移出 `cards-official.js`，以 `sourceType: none`／`status: unknown` 保存喺 registry 嘅 `modelAssumptions`，再由 aggregator 兼容注入，確保純搬遷唔改排序或 UI。

目前弱點：大部分來源狀態係整卡級，未完全做到每個數字獨立來源、有效期及衝突狀態。

### 5.2 渠道優惠

`data/card-channels.js#records` 以卡 ID 關聯平台加碼，保存 `sourceUrl`、`sourceType`、`verifiedAt`、`validFrom`、`validUntil`、`status` 同條件；aggregator 只將 `runtimeOffer` records 映射成舊 `CHANNEL_OFFERS` shape。舊 note 中遺漏嘅 MoneySmart、DBS MoneyHero 等已成獨立 `unknown` records；官方推廣 claim 放喺 `source-registry.js#promotions`，唔用第三方頁證明銀行規則。Phase 1A 為維持 v6.79.0 runtime 文字零 drift，舊有 mixed presentation prose 暫時隔離於渠道檔嘅 `legacyRuntimeText`，再由 aggregator 兼容重組。呢個係待 Stage B 清理嘅 presentation debt。過期狀態由香港日期 helper 判定。
...
Phase 0 額外喺獨立 temp copy 實跑兩個 generator，再比較 `cards/**`／`share/**` 前後 hash；check 由 `node scripts/regression-lock.js` 執行，唔會覆寫工作樹生成頁。
...
- 卡頁 generator 同 freshness audit 已直接 import `data/`；core 本身仍然內嵌 `index.html`，屬下一步引擎拆分範圍。
- 卡來源粒度未足夠細。
- 文章正文唔係真正靜態 SEO 頁。
