...
最後更新：2026-07-23
基準版本：正式 v6.79.0＋已合併 Phase 1 data sources；本 branch 只加入 Phase 2A shell UI 候選
...
- 以下係**已上線現況**，唔等於 2026-07-22 目標 IA 已完成：
...
- Phase 2A 候選 bottom nav 已改為 `點賺／點用／首頁／優惠／攻略`，沿用原有 `opt／redeem／journey／articles／guides` tab IDs；點賺／點用內部 UI、Homepage Hero／Greeting 同 Planner gateway 不在 Phase 2A scope。
...
- GitHub Pages 繼續由 `main` 部署；Draft PR 本身唔會改 production。
...
`verify.js` 覆蓋：語法／HTML、optimizer invariant、版本一致、資料可信度、私隱、RTW、資產、分享、無障礙基本結構及快照。`regression-lock.js` 另外鎖定 22 個 optimizer 情境、正式產品檔 hash、DOM、localStorage 同 generators 零 drift；fixture 只記錄 v6.79.0 現行結果，唔代表數字已獲新一輪官方核實。
