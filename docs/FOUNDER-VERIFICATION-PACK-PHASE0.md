...
### Phase 0 修正覆核（2026-07-22）

今次只修正 Founder 指定嘅 Regression Lock 同 canonical 文件描述，冇擴大 Phase 0 scope：

- `scripts/regression-lock.js` 改為直接讀取 `ACREMILES_STORAGE_KEYS`，兼容 `var`／`let`／`const`。
- 已加入硬性 assertion：reset key 清單不可為空；所有 `index.html` 實際引用嘅 `bm_*` keys 必須全部被清單涵蓋；有任何遺漏即 `FAIL regression-lock`。
- localStorage snapshot 新舊差異：`resetKeys` 由 `[]`（0 個）改為 16 個；`keyCount`、`keys`、`referenceCounts` 同 schemas 冇改。
- 新 `resetKeys`：`bm_card_overrides`、`bm_consent`、`bm_daily`、`bm_esub`、`bm_favs`、`bm_input`、`bm_ok`、`bm_ow`、`bm_rsub`、`bm_sa`、`bm_saved_plans`、`bm_saved_redeem`、`bm_sub`、`bm_theme`、`bm_welcome_ts`、`bm_zoom`。
- D-06 已按實碼修正：2 秒後開始淡出、CSS 過渡 0.45 秒、約 0.5 秒後移除，總約 2.5 秒。
- D-09 已修正為「有衝突」：現行仍有 Greeting，待後續 UI Phase 完全移除。
- MASTER 優惠定位已修正為：「優惠負責所有會影響點賺／點用結果嘅限時資訊。」
...
| D-06 | Welcome 用新 slogan、移除「誠・里／香港里數優化」、0.8–1.2 秒 | 有衝突 | 現行 Welcome 仍有舊 tag；2 秒後開始淡出，CSS 過渡為 0.45 秒，約 0.5 秒後移除（總約 2.5 秒），長過目標 0.8–1.2 秒 |
...
| D-09 | 首頁暫時完全取消 Greeting | 有衝突 | 現行仍有 Greeting，待後續 UI Phase 完全移除。 |
...
| localStorage reset snapshot 重建 | 舊 snapshot 正確觸發 storage drift；重建後 `resetKeys` 16／16、5／5 snapshot groups PASS |
